...
@interface CMyRental ()
@end
@implementation CMyRental

NSManagedObjectContext *contextRental;
NSString* entityNameRental = @"CMyRental";
...
-(instancetype)initWithDictionary:(NSDictionary *)dictionary context:(NSManagedObjectContext *)context
{
	...
	NSEntityDescription * entityDescription = [NSEntityDescription entityForName:@"CMyRental" inManagedObjectContext:context];
...
{
    [self deleteAllDataRental];
    
    NSError *error;
    for (int i = 0; i<[res[@"result"] count]; i++) {
        CMyRental *obj_cl = [NSEntityDescription insertNewObjectForEntityForName:entityNameRental inManagedObjectContext:[self getContext]];
...
-(void)exciteLines:(CMyRental *)obj_dv dic:(NSDictionary *)res
{
...
{
    NSEntityDescription *entityDesc = [NSEntityDescription entityForName:entityNameRental
...
-(NSManagedObjectContext*) getContext{
    if(contextRental==nil){
        contextRental=[kAppDelegate managedObjectContext];
    }
    return contextRental;
}
