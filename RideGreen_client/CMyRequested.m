...
@interface CMyRequested ()
@end
@implementation CMyRequested

NSManagedObjectContext *contextRequested;
NSString* entityNameRequested = @"CMyRequested";
...
-(instancetype)initWithDictionary:(NSDictionary *)dictionary context:(NSManagedObjectContext *)context
{
	...
	NSEntityDescription * entityDescription = [NSEntityDescription entityForName:@"CMyRequested" inManagedObjectContext:context];
...
{
    [self deleteAllDataRequested];
    
    NSError *error;
    for (int i = 0; i<[res[@"result"] count]; i++) {
        CMyRequested *obj_cl = [NSEntityDescription insertNewObjectForEntityForName:entityNameRequested inManagedObjectContext:[self getContext]];
...
-(void)exciteLines:(CMyRequested *)obj_dv dic:(NSDictionary *)res
{
...
{
    NSEntityDescription *entityDesc = [NSEntityDescription entityForName:entityNameRequested
...
-(NSManagedObjectContext*) getContext{
    if(contextRequested==nil){
        contextRequested=[kAppDelegate managedObjectContext];
    }
